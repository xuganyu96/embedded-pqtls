 ...
 *
 * NOTES:
 * ./asn1 --pem --indent --no-dump-text --branch certs/root.crt
 */
...
static int asn1App_ReadFile(FILE *fp, unsigned char **pdata, word32 *plen) {
  ...
  unsigned char *data =
      (unsigned char *)XMALLOC(DATA_INC_LEN, NULL, DYNAMIC_TYPE_TMP_BUFFER);
...
static int FindPem(unsigned char *data, word32 offset, word32 len,
  ...
  if (ret == 0) {
    /* Confirm header. */
    for (i += 10; i < len; i++) {
      if ((data[i] == '-') && (strncmp((char *)data + i, "-----", 5) == 0)) {
        break;
      }
    }
    if (i == len) {
  ...
  if (ret == 0) {
    ...
    i += 6;
    for (j = i + 1; j < len; j++) {
      if ((data[j] == '-') && (strncmp((char *)data + j, "-----END", 8) == 0)) {
        break;
      }
    }
    if (j == len) {
...
int main(int argc, char *argv[]) {
  ...
  while (argc > 0) {
    ...
    if ((strcmp(argv[0], "-b") == 0) || (strcmp(argv[0], "--branch") == 0)) {
    ...
    else if ((strcmp(argv[0], "-O") == 0) || (strcmp(argv[0], "--oid") == 0)) {
      wc_Asn1PrintOptions_Set(&opts, ASN1_PRINT_OPT_SHOW_OID, 1);
    }
#ifndef NO_CODING
    /* File contains PEM blocks. */
    else if ((strcmp(argv[0], "-p") == 0) || (strcmp(argv[0], "--pem") == 0)) {
