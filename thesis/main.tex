...
Figure \ref{fig:classical-tls13-handshake} illustrates a simple TLS 1.3 handshake over TCP. A new TLS 1.3 connection begins with a handshake in which the two parties exchange a specific sequence of messages to negotiate cryptographic parameters, establish shared secrets, and authenticate each other.
...
As illustrated in Figure~\ref{fig:classical-tls13-handshake}, all handshake messages following \texttt{ServerHello}, as well as all application data, are encrypted using different symmetric keys.
...
{\color{red} Draw a diagram of the KEMTLS handshake, describe the workflow verbally, compare with TLS 1.3}

An overview of a unilaterally authenticated KEMTLS handshake is illustrated on Figure \ref{fig:kemtls-handshake}.
...
{\color{red}
KEMTLS and TLS 1.3 have identical security goals but achieve them in different ways, formal analysis first presented in which paper, a machine-assisted proof presented in which other paper.

Subtle security difference: implicit authentication, downgrade resilience, anonymity, deniability vs non-repudiation
}

\subsection{KEMTLS performance}\label{sec:kemtls-performance}
{\color{red} present performance data from prior results: desktop, PDK, IoT}
